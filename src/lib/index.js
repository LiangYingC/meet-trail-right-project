...
export const APP = {
  getDay: () => {
    ...
    const todayDate = `${today.getFullYear()}-${`0${today.getMonth() + 1}`.slice(-2)}-${(
      '0' + today.getDate()
    ).slice(-2)}`;
    return todayDate;
  },
  ...
  getTime: () => {
    const today = new Date();
    const todayTime = `${('0' + today.getHours()).slice(-2)}:${('0' + today.getMinutes()).slice(
      -2
    )}`;
    return todayTime;
  },
    ...
    firebase
      ...
      .then(data => {
        ...
        DB.ref('users').doc(user.uid).set({
          ...
          email: user.email,
          picture:
            'https://firebasestorage.googleapis.com/v0/b/meet-trail-right.appspot.com/o/projectPictures%2FlogoIcon%2FUserlogo.png?alt=media&token=38796a9b-348b-4995-82ee-6b6c88f70eb8',
          timestamp: DB.time(),
          status: '享受悠遊山林步道的時光',
          like_list: [],
          create_list: [],
