...
class Report extends Component {
  ...
  render() {
    const { dateValue, reportList, isShowReportInputBox, isShowLoginBox } = this.state;
      ...
      <>
                  ...
                  <div className="report-item" key={index}>
                    <div className="flex report-info">
                      <div className="report-user-img">
                        <img
                          src={reportItem.createUser.picture}
                          alt={`${reportItem.createUser.name}的照片`}
                        />
                      </div>
        ...
        <div className={`report-input ${isShowReportInputBox ? 'active' : ''}`}>
          ...
          <div className={`report-input-box ${isShowReportInputBox ? 'active' : ''}`}>
            <h3>步道狀況回報</h3>
            <div className="block">
              <label htmlFor="input-report-date">發生日期</label>
              <input
                type="date"
            ...
            <div className="block">
              <label htmlFor="input-report-content">回報內容</label>
              <textarea
            ...
            <Button text={'確認送出'} id={'send-report-btn'} onClick={this.setReportData} />
            <div className="close-btn" onClick={this.toggleReportInputBox}></div>
          </div>
        </div>

        <LoginBox isShowLoginBox={isShowLoginBox} closeLoginBox={this.closeLoginBox} />
      </>
    );
  }
}
