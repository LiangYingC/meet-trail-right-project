...
class Trails extends Component {
  ...
  setVisibleList = () => {
    ...
    this.setState(preState => ({
      trailsVisible: preState.trailsAll.filter(trail => {
        ...
        const difficultyCheckedName =
          filterListConst[1].optionList[difficultyFilterChecked.value].name;
        ...
        const lengthCheckedMinValue =
          filterListConst[3].optionList[lengthFilterChecked.value].minValue;
        const lengthCheckedMaxValue =
          trailLengthValue < filterListConst[3].optionList[lengthFilterChecked.value].maxValue;

        // Filter visibale trail
        if (
          (trailAreaName === areaCheckedName || areaFilterChecked.value === 0) &&
          (trailDifficultyName === difficultyCheckedName || difficultyFilterChecked.value === 0) &&
          ((timeCheckedMinValue <= trailTimeValue && trailTimeValue < timeCheckedMaxValue) ||
            timeFilterChecked.value === 0) &&
          ((lengthCheckedMinValue <= trailLengthValue &&
            trailLengthValue < lengthCheckedMaxValue) ||
            lengthFilterChecked.value === 0)
        ) {
          return trail;
        }
      }),
  ...
  render() {
    const { trailsVisible, filterCheckedList, sortCheckedValue } = this.state;
