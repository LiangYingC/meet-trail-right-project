...
describe('Test <TrailSort />', () => {
  ...
  test('test sorts quantity', () => {
    ...
    const { container } = render(
      <TrailSort changeSort={changeSort} sortCheckedValue={sortCheckedValue} />
    );
  ...
  test('test initial active sort', () => {
    ...
    expect(container.querySelector('.sort-item.active').textContent).toBe(
      TrailsConst.sortList[0].option
    );
  });
  ...
  test('test last active sort', () => {
    ...
    expect(container.querySelector('.sort-item.active').textContent).toBe(
      TrailsConst.sortList[TrailsConst.sortList.length - 1].option
    );
  });
});
