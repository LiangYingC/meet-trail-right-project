...
class TrailCreate extends Component {
  ...
  checkInputsIsOkOrNot = () => {
    ...
    const processRadioOrCheckbox = (keyList, dataList) => {
      const trueKeyList = Object.keys(keyList).filter(key => keyList[key]);
    ...
    inputKeyList.forEach(key => {
      if (key !== 'routeImg') {
        // routeImg is not required to fill
        !inputValue[key]
          ? TurnInputsCheckedFalse()
          : this.checkValueType(key, inputValue[key], TurnInputsCheckedFalse);
      }
    });
  ...
  createTrail = (inputValue, sceneryData, difficultyData) => {
    ...
    DB.ref('trails')
      .add(APP.tansformTrialDataFromStateToDB(inputValue, sceneryData, difficultyData, userData))
      .then(newTrail => {
        const newCreateList = userData.createList;
        newCreateList.push(newTrail.id);

        DB.ref('users').doc(userData.id).update({
  ...
  render() {
    const { inputValue, alterBox, alterWord, isShowImgLoading, isShowCreateLoading } = this.state;

    const { history } = this.props;

    const { locationObj, difficultyList, sceneryList } = TrailCreateConst;
          ...
          <div className="wrap">
            ...
            <UploadImgInput
              ...
              reminderContent={
                <>
                  <p>
                    1. 此<span>封面圖</span>會呈現在步道頁面最上方
                  </p>
                  <p>
                    2. 建議圖片尺寸比例為<span> 3 : 2 </span>
                  </p>
                  <p>
                    3. 建議圖片寬度至少大於<span> 800 像素 </span>
                  </p>
                  <p>
                    4. 檔案須小於<span> 5 MB</span>
                  </p>
                </>
              }
            />
            ...
            <UploadImgInput
              ...
              reminderContent={
                <>
                  <p>
                    1. 此<span>路線圖</span>，呈現在步道頁面<span>路線資訊</span>處
                  </p>
                  <p>
                    2. 建議圖片尺寸比例為<span> 3 : 2 </span>
                  </p>
            ...
            <div className="confirm-btns">
              ...
              />
              <Button text={'清空全部'} id={'clear-all-btn'} onClick={this.clearAllInputs} />
            </div>
          </div>
        </div>
        <Alter
