...
const LikeButton = ({ trailId, toggleLoginBox }) => {
  const updateUserLikeData = (userid, newLikeList) => {
    DB.ref('users').doc(userid).update({
      ...
      if (isLiked) {
        ...
        DB.ref('trails')
          ...
          .then(doc => {
            ...
            const newTrailLikeUserList = trailLikeUserList.filter(
              likeUserId => likeUserId !== userData.id
            );
      ...
      {({ userData, isLogin }) => {
        ...
        return (
          <div className="like-button">
            <i
              className={`far fa-heart ${isLiked ? 'active' : ''}`}
